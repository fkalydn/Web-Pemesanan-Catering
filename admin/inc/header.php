...
<html lang="en">
  ...
  $qpesanan = $koneksi->query("SELECT * FROM pesanan where `read`='0'") or die (mysqli_error());
  $totalUnRead = mysqli_num_rows($qpesanan);
  // TOTAL PEMBAYARAN YANG BELUM DI VERIFIKASI
  $qPembayaran = $koneksi->query("SELECT * FROM pembayaran where `status`='pending'") or die (mysqli_error());
