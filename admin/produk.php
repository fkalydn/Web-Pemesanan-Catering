<?php 
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = $koneksi->query("DELETE FROM produk WHERE id='$_GET[id]'");
			if($q){ alert("Hapus data sukses"); redir("produk.php"); }
		}  
	}
	// Query Tambah
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			...
			$q = $koneksi->query("INSERT INTO produk Values(NULL,'$nama','$deskripsi','$gambar','$harga','$kategori_produk_id')");
			if($q){
				$upload = move_uploaded_file($_FILES['gambar']['tmp_name'], '../uploads/'.$gambar);
				if($upload){ alert("Tambah data sukses"); redir("produk.php"); }
			}
		}
	}
	// Query edit
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){ 
			...
			$q = $koneksi->query("UPDATE produk SET nama='$nama',deskripsi='$deskripsi', gambar='$gambar', harga='$harga', kategori_produk_id='$kategori_produk_id' where id=$_GET[id]") or die(mysqli_error());
			if($q){
				$upload = move_uploaded_file($_FILES['gambar']['tmp_name'], '../uploads/'.$gambar);
				if($upload){ alert("Edit data sukses"); redir("produk.php"); }
			}
		}
	}
	...
	<div class="container">
		...
		<?php
			if(!empty($_GET)){
				...
				?>
					...
					<form action="" method="post" enctype="multipart/form-data">
						...
						<select name="kategori_produk_id" required class="form-control"> 
							<?php
								$katpro = $koneksi->query("SELECT * FROM kategori_produk");
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object($koneksi->query("SELECT * FROM produk where id='$_GET[id]'"));
				?>
					<div class="row col-md-6">
					<form action="produk.php?act=edit&&id=<?php echo $_GET['id'] ?>" method="post" enctype="multipart/form-data">
						...
						<select name="kategori_produk_id" required class="form-control"> 
							<?php
								$katpro = $koneksi->query("SELECT * FROM kategori_produk where id='$data->kategori_produk_id'");
