<?php
	...
	if(!empty($_GET)){
		if($_GET['act'] == 'delete'){
			...
			$q = $koneksi->query("DELETE FROM laporan WHERE id_pengeluaran='$_GET[id]'");
	...
	if(!empty($_GET['act']) && $_GET['act'] == 'create'){
		if(!empty($_POST)){
			extract($_POST);
			$q = $koneksi->query("INSERT INTO laporan Values(NULL,'$nama_barang','$Tanggal_pengeluaran','$harga','$jumlah',$harga*$jumlah)");
			if($q){ alert("Success"); redir("input_pengeluaran.php"); }
		}
	}
	if(!empty($_GET['act']) && $_GET['act'] == 'edit'){
		if(!empty($_POST)){
			...
			$q = $koneksi->query("UPDATE laporan SET nama_barang='$nama_barang',harga='$harga',jumlah='$jumlah',total='$harga*$jumlah' WHERE id_pengeluaran=$_GET[id]");
			if($q){ alert("Success"); redir("input_pengeluaran.php"); }
		}
	}
...
?>
	...
	<div class="container">
		<?php
			$q = $koneksi->query("SELECT * FROM laporan");
			$j = mysqli_num_rows($q);
		?>
			...
			if(!empty($_GET)){
				...
				?>
					...
					<form action="" method="post" enctype="multipart/form-data">
						...
						<input type="text" class="form-control" name="harga" required placeholder="Masukkan Harga Satuan"><br>
				...
				if($_GET['act'] == 'edit'){
					$data = mysqli_fetch_object($koneksi->query("SELECT * FROM laporan WHERE id_pengeluaran='$_GET[id]'"));
				?>
					<div class="row col-md-6">
